...
int switch_root_main(int argc, char *argv[])
{
    ...
    // if change directory(newroot) fails, OR
    // lstat($PWD) fails, OR lstat(/) fails OR
    // the device for $PWD and the device for / are the same
    ...
    // if lstat(/init) fails OR
    // "not is regular file" for (/init) OR
    // statfs(/) fails
    // the stat call for /init should work, /init should
    // if (lstat("/init", &st1) || !S_ISREG(st1.st_mode) || statfs("/", &stfs) )
	if (lstat("/init", &st1) || statfs("/", &stfs) )
	{
		bb_error_msg_and_die("Huh. stat calls failed for /init");
	}

	if	(stfs.f_type != RAMFS_MAGIC && stfs.f_type != TMPFS_MAGIC) 
	{
		bb_error_msg_and_die("Huh. root filesystem not RAMFS or TMPFS");
	}

    if ( getpid() != 1 )
    {
        bb_error_msg_and_die("Huh. my PID is != 1");
    }
	...
	if (mount(".", "/", NULL, MS_MOVE, NULL) || chroot(".") || chdir("/"))
		bb_error_msg_and_die("moving root failed");
